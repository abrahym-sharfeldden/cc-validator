let checkVal = function(e) {
	...
	return ((keyCode != 46 && keyCode > 31 && (keyCode < 48 || keyCode > 57)) ? false : true);
}
...
let submitVal = function() {
	...
	if(luhnCheck(cardNumber))
		return log.innerHTML = `Card Verified!`;
	else
		return log.innerHTML = `Invalid Card Number!`
}
...
let numFormat = function(v){
	...
	document.getElementById("cc-icon").classList = checkCardType(v) ? `fab fa-3x fa-cc-${checkCardType(v).toLowerCase()}` : "fab fa-3x fa-cc-none";
...
let checkCardType = function(cardNumber){	
	...
	function isMasterCard() {
		...
		function isDiscover(){
			let bool = /^6(?:011\d{12}|5\d{14}|4[4-9]\d{13}|22(?:1(?:2[6-9]|[3-9]\d)|[2-8]\d{2}|9(?:[01]\d|2[0-5]))\d{10})/g
